% POSTPROCESS_BANDED_BIMODAL  Post-process data from test_banded_bimodal.m.
%
%   Unimodal-to-bimodal SB (mass-splitting transport), Crank-Nicolson banded
%   FP projection. Reference is Sinkhorn for every eps (no closed form for
%   mixture marginals). Mirrors postprocess_expsemi_bimodal.m otherwise.
%
...
% -------------------------------------------------------------------------
files = dir(fullfile(dat_dir, 'banded2d_bimodal_eps*_nt*_nx*.mat'));
if isempty(files)
...
for k = 1:NF
    tok = regexp(files(k).name, ...
        'banded2d_bimodal_eps([\d.e+\-]+)_nt(\d+)_nx(\d+)', 'tokens');
    if ~isempty(tok)
        t = tok{1};
...
for k = 1:numel(files)
    ...
    % Sinkhorn can fail to converge/diverge for some eps -- test_banded_bimodal.m
    ...
    savefig_both(fig, fig_dir, ['admm_conv_banded_bimodal_' tag]);
    ...
    savefig_both(fig, fig_dir, ['err_vs_t_banded_bimodal_' tag]);
    end
    ...
    sgtitle(sprintf('Density ($\\varepsilon=%.4g$,  $N_T=%d$,  $N_x=%d$)', ...
        d.eps_i, d.NT, d.NX), 'FontSize', FS+1, 'Interpreter', 'latex');
    savefig_both(fig, fig_dir, ['density_banded_bimodal_' tag]);
    ...
    set(gca,'FontSize',FS,'Box','on','TickDir','out'); grid on;
    savefig_both(fig, fig_dir, ['iter_time_banded_bimodal_' tag]);
    ...
    legend('Location', 'best', 'FontSize', FS-1, 'Box', 'off');
    set(gca, 'FontSize', FS, 'Box', 'on', 'TickDir', 'out');  grid on;
    savefig_both(fig, fig_dir, ['diag_slice_banded_bimodal_' tag]);
...
%% =========================================================================
sfiles = dir(fullfile(dat_dir, 'banded2d_sweep_bimodal_nt*_nx*.mat'));
...
for k = 1:numel(sfiles)
    ...
    savefig_both(fig, fig_dir, ['sweep_banded_bimodal_' tag]);
    close all;
end
...
for ei = 1:numel(EPS_VALS)
    ...
    savefig_both(fig, fig_dir, sprintf('refine_banded_bimodal_eps%g', eps_i));
    close all;
end
