% SWEEP_GAMMA  Sweep over ADMM penalty parameter gamma (spectral ADMM).
%
% Plots:
...
%% --- Parameters ---
gamma_vals = [1, 10, 100, 500];
n_gamma    = numel(gamma_vals);

cfg_base = cfg_admm_spectral_fe();
prob_def = prob_gaussian();
problem  = setup_problem_spectral(cfg_base, prob_def);  % fixed grid for all gamma
...
%% --- Analytical (fixed eps, independent of gamma) ---
[rho_a, mx_a] = analytical_sb_gaussian_spectral(problem, cfg_base.vareps);
ke_ana_val    = compute_objective_spectral(rho_a, mx_a, problem) / problem.dx;
...
for k = 1:n_gamma
    ...
    % precomp_spectral_proj does not depend on gamma; reuse same problem
    fprintf('[%d/%d]  gamma=%g ... ', k, n_gamma, cfg_k.gamma);
    r = cfg_k.pipeline(cfg_k, problem);
    fprintf('iters=%d  error=%.2e  time=%.1fs\n', r.iters, r.error, r.walltime);

    ke_admm(k) = compute_objective_spectral(r.rho, r.mx, problem) / problem.dx;
    results{k} = r;
end

%% --- Plot 1: density evolution per gamma ---
...
for k = 1:n_gamma
    ...
    title(sprintf('$\\gamma=%g$', gamma_vals(k)), 'Interpreter', 'latex');
...
title(sprintf('ADMM residual   $\\varepsilon=%g$', cfg_base.vareps), 'Interpreter', 'latex');
legend('Interpreter', 'latex', 'Location', 'northeast', 'FontSize', 9);
grid on;
saveas(gcf, fullfile(fig_dir, sprintf('residual_%s.png', ftag)));

%% --- Summary ---
fprintf('\n%-10s  %-8s  %-12s  %-12s\n', 'gamma', 'iters', 'KE (spec)', 'KE (ana)');
fprintf('%s\n', repmat('-', 1, 48));
for k = 1:n_gamma
    fprintf('%-10g  %-8d  %-12.6f  %-12.6f\n', ...
        gamma_vals(k), results{k}.iters, ke_admm(k), ke_ana_val);
end
