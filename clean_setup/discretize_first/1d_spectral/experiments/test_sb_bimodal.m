% TEST_SB_BIMODAL  SB between two bimodal distributions (spectral ADMM).
...
figure('Name', 'SB bimodal density', 'Position', [100 100 700 400]);
hold on;
for p = 1:n_t
...
title(sprintf('SB bimodal   $\\varepsilon=%.4g$', cfg.vareps), 'Interpreter', 'latex');
grid on;
saveas(gcf, fullfile(fig_dir, sprintf('density_sb_bimodal_%s.png', ftag)));
...
grid on;
saveas(gcf, fullfile(fig_dir, sprintf('residual_sb_bimodal_%s.png', ftag)));
