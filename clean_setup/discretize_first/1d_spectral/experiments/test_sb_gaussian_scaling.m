% TEST_SB_GAUSSIAN_SCALING  SB between scaled Gaussians (spectral ADMM).
%
%   Tests a Gaussian whose mean or variance changes between t=0 and t=T.
%   Compares against analytical solution if available.
...
try
    [rho_ana, ~] = analytical_sb_gaussian_general(problem, cfg.vareps);
    has_ana = true;
catch
    has_ana = false;
end
...
figure('Name', 'SB Gaussian scaling', 'Position', [100 100 700 400]);
hold on;
for p = 1:n_t
...
title(sprintf('SB Gaussian scaling   $\\varepsilon=%.4g$', cfg.vareps), 'Interpreter', 'latex');
grid on;
saveas(gcf, fullfile(fig_dir, sprintf('density_sb_gaussian_scaling_%s.png', ftag)));
...
grid on;
saveas(gcf, fullfile(fig_dir, sprintf('residual_sb_gaussian_scaling_%s.png', ftag)));
