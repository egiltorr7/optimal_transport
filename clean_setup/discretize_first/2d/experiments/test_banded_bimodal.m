% TEST_BANDED_BIMODAL_2D  Crank-Nicolson banded ADMM vs Sinkhorn reference on
%   unimodal-to-bimodal SB (mass-splitting transport). Mirrors
%   test_expsemi_bimodal.m but uses the CN banded FP projection
%   (proj_fokker_planck_banded) instead of ETD.
%
%   Saves one .mat per (grid, eps) and one sweep summary per grid.
%   No figures — post-process locally with postprocess_banded_bimodal.m.
%
%   Marginals:
...
%   no closed-form SB solution exists for mixture marginals.
%
%   Output files:
%     results/data/bimodal/banded2d_bimodal_eps<e>_nt<N>_nx<N>.mat  (per run)
%     results/data/bimodal/banded2d_sweep_bimodal_nt<N>_nx<N>.mat   (per grid)
...
for g = 1:N_GRIDS
    ...
    for i = 1:NE
        ...
        fname = sprintf('banded2d_bimodal_eps%g_nt%d_nx%d.mat', eps_i, NT, NX);
    ...
    sfname = sprintf('banded2d_sweep_bimodal_nt%d_nx%d.mat', NT, NX);
