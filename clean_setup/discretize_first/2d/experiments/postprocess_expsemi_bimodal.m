% POSTPROCESS_EXPSEMI_BIMODAL  Post-process data from test_expsemi_bimodal.m.
%
%   Unimodal-to-bimodal SB (mass-splitting transport). Reference is Sinkhorn
%   for every eps (no closed form for mixture marginals). Mirrors
%   postprocess_expsemi.m otherwise; see that file for the full figure list.
%
%   Run this locally after copying results/data/ from the server.
%   Reads all expsemi2d_bimodal_*.mat files found in dat_dir and produces:
%
...
dat_dir  = fullfile(base_dir, '..', 'results', 'data', 'bimodal');
fig_dir  = fullfile(base_dir, '..', 'results', 'figures','paper', 'bimodal');
...
% -------------------------------------------------------------------------
files = dir(fullfile(dat_dir, 'expsemi2d_bimodal_eps*_nt*_nx*.mat'));
if isempty(files)
...
for k = 1:NF
    tok = regexp(files(k).name, ...
        'expsemi2d_bimodal_eps([\d.e+\-]+)_nt(\d+)_nx(\d+)', 'tokens');
    if ~isempty(tok)
        t = tok{1};
...
for k = 1:numel(files)
    ...
    % Sinkhorn can fail to converge/diverge for some eps -- test_expsemi_bimodal.m
    % saves rho_ref_*/err_*_t as [] in that case rather than a broken reference.
    has_ref = ~isempty(d.rho_ref_cc);
    if ~has_ref
        fprintf('  (no valid Sinkhorn reference for this eps -- skipping error-vs-time plot,\n');
        fprintf('   diagonal-slice reference line and marker will still be shown)\n');
    end
    ...
    savefig_both(fig, fig_dir, ['admm_conv_bimodal_' tag]);

    %% --- (2) Error vs time (skipped if no valid Sinkhorn reference) ------
    ...
    savefig_both(fig, fig_dir, ['err_vs_t_bimodal_' tag]);
    end
    ...
    % rho1 is a two-bump mixture (unimodal-to-bimodal test case)
    rho1_2d = 0.5*normal2d_density(d.xx, d.yy, d.MU1A(1), d.MU1A(2), d.SIGMA1) ...
            + 0.5*normal2d_density(d.xx, d.yy, d.MU1B(1), d.MU1B(2), d.SIGMA1);
    ...
    sgtitle(sprintf('Density ($\\varepsilon=%.4g$,  $N_T=%d$,  $N_x=%d$)', ...
        d.eps_i, d.NT, d.NX), 'FontSize', FS+1, 'Interpreter', 'latex');
    savefig_both(fig, fig_dir, ['density_bimodal_' tag]);
    ...
    set(gca,'FontSize',FS,'Box','on','TickDir','out'); grid on;
    savefig_both(fig, fig_dir, ['iter_time_bimodal_' tag]);
    ...
    legend('Location', 'best', 'FontSize', FS-1, 'Box', 'off');
    set(gca, 'FontSize', FS, 'Box', 'on', 'TickDir', 'out');  grid on;
    savefig_both(fig, fig_dir, ['diag_slice_bimodal_' tag]);
...
%% =========================================================================
sfiles = dir(fullfile(dat_dir, 'expsemi2d_sweep_bimodal_nt*_nx*.mat'));
...
for k = 1:numel(sfiles)
    ...
    savefig_both(fig, fig_dir, ['sweep_bimodal_' tag]);
    close all;
end
...
for ei = 1:numel(EPS_VALS)
    ...
    savefig_both(fig, fig_dir, sprintf('refine_bimodal_eps%g', eps_i));
    close all;
end
