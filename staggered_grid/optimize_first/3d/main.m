...
%% Grid
% CFL constraint
vareps  = 0.3;      % Schrödinger regularisation strength
nx  = 128;
nt = 64;
...
%% Test-case settings
test_case         = 'gaussian';
compute_reference = true;    % run a long solve and save as reference
...
%   figures/<test_case>/nx<nx>_nt<nt>_gamma<gamma>_eps<vareps>/
%   Example: figures/gaussian/nx128_nt64_gamma10_eps1e-6/
eps_str  = regexprep(sprintf('%.0e', vareps), 'e([+-])0+(\d)', 'e$1$2');
fig_dir  = fullfile('figures', test_case, ...
               sprintf('nx%d_nt%d_gamma%g_eps%s', nx, nt, gamma, eps_str));
...
if has_exact
    ...
    legend(ax, '$\rho_{\mathrm{ADMM}}$', '$\rho_{\mathrm{SB}}$ (exact)', ...
...
for k = 1:nt-1
    ...
    title(sprintf('Density transport, $t = %d/%d$', k, nt));
    drawnow;
...
if has_true_error
    ...
    title('Error vs.\ reference solution');
    grid on;
else
    fig = new_fig(sty.fw, sty.fh1);
    semilogy(2:maxIter, outs_admm.residual_diff(2:end), ...
